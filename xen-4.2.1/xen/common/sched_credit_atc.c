...
#define CSCHED_CREDITS_PER_MSEC     1000
 ...
 #define CSCHED_TIME_APPLY (21000)
 #define ALPHA 4
...
{
    unsigned time_slice;
    unsigned long long spinlock_log;
};
...
struct csched_dom {
    ...
    uint64_t prev_spinlock_count;
    unsigned tslice_us;
    unsigned state;
    struct spin_state sstate[4];
    int count;
    int zero_count;
};
...
struct csched_private {
    ...
    struct timer time_slice_ticker;
};
...
extern int do_vcrd_op(unsigned long long time, int lock)
{
    ...
    sdom->spinlock = (sdom->spinlock) / ALPHA + time / ALPHA * (ALPHA - 1);
    sdom->spinlock_count++;
    ...
    if(dom->domain_id == 1)
    {
        total_time += time;
        count++;
        //printk(XENLOG_WARNING "time:%lld, total_time:%lld, total_count:%lld time slice:%d\n",
		//		time, total_time, count, global_prv->tslice_us);
    }
    /*sdom->spinlock = time;*/
    return 1;
}

extern void do_data_op(int domain_id, int packets)
{
...
static int log(uint64_t number)
{
    /*uint64_t origins[] = {64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576};*/
    /*uint64_t origins[] = {64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768};*/
    uint64_t origins[] = {1024, 1536, 2048, 3072, 4096, 6144, 8192, 12288, 16384, 32768};
    unsigned int i;
...
/*static void update_vcpu_credits(int old_time_slice, int new_time_slice)
{
    struct list_head *iter_sdom, *next_sdom;
    struct list_head *iter_vcpu, *next_vcpu;
    struct csched_private *prv = global_prv;

    int old_credits_per_slice = old_time_slice * CSCHED_CREDIT_PER_US;
    int new_credits_per_slice = new_time_slice * CSCHED_CREDIT_PER_US;
    int old_credits, new_credits;
    struct csched_dom *sdom;
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
        ...
        list_for_each_safe( iter_vcpu, next_vcpu, &sdom->active_vcpu )
        {
            ...
            old_credits = atomic_read(&svc->credit);
            new_credits = old_credits * new_credits_per_slice / old_credits_per_slice;
            atomic_set(&svc->credit, new_credits);
        }  
...
{ 
    uint8_t spinlock_log;
    unsigned tslice_us;

    spinlock_log = log(sdom->spinlock);

    if ((sdom->spinlock_count <= 1) && (spinlock_log == 1))
    {
	sdom->zero_count++;
        /*if (sdom->zero_count % 2 == 0)
        {*/
            sdom->prev_spinlock_count = sdom->spinlock_count;
            sdom->spinlock_count = 0;
            sdom->spinlock = 0;

            if (sdom->tslice_us < 29500)
            {
                sdom->tslice_us = sdom->tslice_us + 500;
            }
            else
            {
                sdom->tslice_us = 30000;
            }

            if (sdom->count == 0)
            {
                sdom->sstate[0] = sdom->sstate[1];
                sdom->sstate[1] = sdom->sstate[2];
                sdom->sstate[2] = sdom->sstate[3];
                sdom->sstate[3].spinlock_log = spinlock_log;
                sdom->sstate[3].time_slice = tslice_us;
            }
			
			if(sdom->dom->domain_id == 4) {
				printk(XENLOG_WARNING "domain_id:%d\tstate 0:\t%lld\tstate 1:\t%lld\tstate 2:\t%lld\tcurr:\t%d\tslice:\t%u\n", 
					sdom->dom->domain_id, sdom->sstate[0].spinlock_log, sdom->sstate[1].spinlock_log,
					sdom->sstate[2].spinlock_log, spinlock_log, sdom->tslice_us);
			}
            return;
        /*}8*/
    }
        ...
        if(sdom->count > 0)
        {
            ...
            else if(spinlock_log <= 15)
            {
                tslice_us = 49980 - 3300 * spinlock_log;
            }
            else
            {
                tslice_us = 300;
            }
            sdom->sstate[3-sdom->count].spinlock_log = spinlock_log;
            sdom->sstate[3-sdom->count].time_slice = tslice_us;
            sdom->count--;
        }
        else
        {
            if(sdom->sstate[2].spinlock_log < spinlock_log)
            {
                sdom->sstate[0] = sdom->sstate[1];
                sdom->sstate[1] = sdom->sstate[2];
                sdom->sstate[2] = sdom->sstate[3];
                sdom->sstate[3].spinlock_log = spinlock_log;
                    if(sdom->sstate[2].time_slice >= 1300)
                    {
                        sdom->sstate[3].time_slice = sdom->sstate[2].time_slice - 1000;
                    }
                    else
                    {
                        sdom->sstate[3].time_slice = 300;
                    }
                    tslice_us = sdom->sstate[3].time_slice;
            }
            else if(sdom->sstate[2].spinlock_log == spinlock_log)
            {
                /*if(sdom->dom->domain_id == 1)
                {
                    printk(XENLOG_WARNING "2, [0].spinlock:%lld, [0].time_slice:%d, [1].spinlock:%lld, [1].time_slice:%d, [2].spinlock:%lld, [2].time_slice:%d\n", sdom->sstate[0].spinlock_log, sdom->sstate[0].time_slice, sdom->sstate[1].spinlock_log, sdom->sstate[1].time_slice, sdom->sstate[2].spinlock_log, sdom->sstate[2].time_slice);
                } */  
                sdom->sstate[0] = sdom->sstate[1];
                sdom->sstate[1] = sdom->sstate[2];
                sdom->sstate[2] = sdom->sstate[3];
                sdom->sstate[3].spinlock_log = spinlock_log;
                sdom->sstate[3].time_slice = sdom->sstate[2].time_slice;
                tslice_us = sdom->sstate[3].time_slice;
            }
            else
            {
                if((sdom->sstate[0].spinlock_log >= sdom->sstate[1].spinlock_log) && (sdom->sstate[1].time_slice >= sdom->sstate[2].time_slice))
                {
                    /*if(sdom->dom->domain_id == 1)
                        printk(XENLOG_WARNING "3\n");*/
                    sdom->sstate[0] = sdom->sstate[1];
                    sdom->sstate[1] = sdom->sstate[2];
                    sdom->sstate[2] = sdom->sstate[3];
                    sdom->sstate[3].spinlock_log = spinlock_log;
                ...
                {
                    /*if(sdom->dom->domain_id == 1)
                        printk(XENLOG_WARNING "4");*/
                    sdom->sstate[0] = sdom->sstate[1];
                    sdom->sstate[1] = sdom->sstate[2];
                    sdom->sstate[2] = sdom->sstate[3];
                    sdom->sstate[3].spinlock_log = spinlock_log;
                    //printk(XENLOG_WARNING "tslice_us:%u\n", sdom->sstate[2].time_slice);
                    /*if(sdom->sstate[2].time_slice >= 29000)
                    {
                        sdom->sstate[3].time_slice = sdom->sstate[2].time_slice + 1000;
                    }
                    else
                    {
                        sdom->sstate[3].time_slice = 30000;
                    }*/
                    sdom->sstate[3].time_slice = sdom->sstate[2].time_slice;
                    tslice_us = sdom->sstate[3].time_slice;
                    //printk(XENLOG_WARNING "tslice_us:%u\n", tslice_us);
    ...
    /*if(tslice_us > global_prv->tslice_us)
    {
        if(sdom->spinlock_count >= sdom->prev_spinlock_count)
        {
            sdom->state = 0;
            tslice_us = global_prv->tslice_us;
        }
        else if(sdom->state < 3)
        {
            tslice_us = global_prv->tslice_us;
            sdom->state++;
        }
...
static void csched_update_acct(void)
{
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
        sdom = list_entry(iter_sdom, struct csched_dom, active_sdom_elem);     
		active_count++;
        update_time_slice(sdom);

        if(sdom->tslice_us < min)
        {
            min = sdom->tslice_us;
        }    

    }
//	printk(XENLOG_WARNING "active domain number: %d\n", active_count);

    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
        sdom = list_entry(iter_sdom, struct csched_dom, active_sdom_elem);
        sdom->tslice_us = min;
        sdom->sstate[3].time_slice = min;
    }

    /*update_vcpu_credits(prv->tslice_us, min);*/
    ...
    //printk(XENLOG_WARNING "time_slice:%d\n", prv->tslice_us);
}

static void csched_dynamic_time_slice(void *prv)
{
    /*struct csched_dom *current_dom;
    uint64_t spinlock;
    unsigned int tslice_us;
    uint64_t spinlock_log;
    if(is_idle_domain((struct domain *)dom))
    {
        return;
    }

    current_dom = CSCHED_DOM((struct domain *)dom);
    spinlock = current_dom->spinlock;
    spinlock_log = log(spinlock);
    current_dom->spinlock = 0;*/
    ...
    unsigned long flags;
    spin_lock_irqsave(&global_prv->lock, flags);
    
    /*update the account time for master cpu*/
    csched_update_acct();
    spin_unlock_irqrestore(&global_prv->lock, flags);

    set_timer( &global_prv->time_slice_ticker,
               NOW() + MICROSECS(CSCHED_TIME_APPLY));
...
void set_vcpu_affinity(struct vcpu *vc)
{
    ...
    cpumask_clear(&used_cpus);

    for_each_vcpu(dom, vcpu)
    {
        used_cpu = vcpu->processor;
        cpumask_set_cpu(used_cpu, &used_cpus);
    }

    for_each_vcpu(dom, vcpu)
    {
        cpumask_andnot((vcpu->cpu_affinity), &cpu_online_map, &used_cpus);
        cpumask_set_cpu(vcpu->processor, (vcpu->cpu_affinity));
    }
}

static void get_cpu_runnable_tasks(int cpu)
{
    struct list_head *runq = RUNQ(cpu);
    struct csched_pcpu *pc = CSCHED_PCPU(cpu);

    struct list_head *iter;

    spin_lock(&pc->count_lock);
    ...
    spin_unlock(&pc->count_lock);
}

static int choose_least_load_cpu(struct vcpu *vc)
{
    cpumask_t cpus;

    struct csched_pcpu *pc;
    ...
    {
        get_cpu_runnable_tasks(cpu);
    }

   least_load_cpu = cpumask_first(&cpus);
   pc = CSCHED_PCPU(least_load_cpu);
   least_load = pc->runnable_tasks;

   for_each_cpu(cpu, &cpus)
   {
       pc = CSCHED_PCPU(cpu);
       load = pc->runnable_tasks;
       if(load < least_load)
       {
           least_load = load;
           least_load_cpu = cpu;
...
csched_free_pdata(const struct scheduler *ops, void *pcpu, int cpu)
{
    ...
    if ( (prv->master == cpu) && (prv->ncpus > 0) )
    {
        ...
        migrate_timer(&prv->time_slice_ticker, prv->master);
    }
    kill_timer(&spc->ticker);
    if ( prv->ncpus == 0 )
        kill_timer(&prv->master_ticker);
        kill_timer(&prv->time_slice_ticker);
...
csched_alloc_pdata(const struct scheduler *ops, int cpu)
{
    ...
    if ( prv->ncpus == 1 )
    {
        ...
        init_timer(&prv->time_slice_ticker, csched_dynamic_time_slice, prv, cpu);
        set_timer(&prv->time_slice_ticker, NOW() + MICROSECS(CSCHED_TIME_APPLY));
    }
    
    /*add*/
    spin_lock_init(&spc->count_lock);
...
_csched_cpu_pick(const struct scheduler *ops, struct vcpu *vc, bool_t commit)
{
    ...
    if((!is_idle_domain(dom)))
    {
        ...
        for_each_vcpu(dom, vcpu)
        {
            ...
            if(vcpu != vc)
            {
                if(vcpu->processor == cpu)
                {
                    /*is there any way better:FIXME*/
                    cpu = vc->processor;
...
csched_dom_init(const struct scheduler *ops, struct domain *dom)
{
    ...
    sdom->spinlock_count = 0;
    sdom->prev_spinlock_count = 0;
    sdom->state = 0;

    sdom->count = 3;
    sdom->zero_count = 0;
    sdom->sstate[0].time_slice = sdom->tslice_us;
...
csched_acct(void* dummy)
{
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
        ...
        /*printk(XENLOG_WARNING "domain_id:%d,credit_fair:%d, credit_total:%d, dom_weight:%d, dom_vcpu_count:%d, weight_total:%d\n",
            sdom->dom->domain_id, credit_fair, credit_total,sdom->weight, sdom->active_vcpu_count, weight_total);*/
        ...
        list_for_each_safe( iter_vcpu, next_vcpu, &sdom->active_vcpu )
        {
            ...
            else
            {
                ...
                {
                    /*printk(XENLOG_WARNING "credit:%d, slice_credit:%d\n", credit, prv->credits_per_tslice);*/
