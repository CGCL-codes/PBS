...
#define tmpstr keyhandler_scratch
    ...
    for_each_domain ( d )
    {
        ...
        for_each_vcpu ( d, v )
        {
            ...
            printk("    %s\n", tmpstr);
			printk("pmuinfo: pmc[0]=%llu    pmc[1]=%llu    pmc[2]=%llu    pmc[3]=%llu\n", v->pmc[0], v->pmc[1], v->pmc[2], v->pmc[3]);
        }
    }
...
/*added*/
static struct keyhandler dump_customized_keyhandler = {
	.diagnostic = 1,
	.u.fn = dump_customized,
	.desc = "dump customized settings"
};
...
void __init initialize_keytable(void)
{
	...
	/*added*/
	register_keyhandler('z', &dump_customized_keyhandler);
