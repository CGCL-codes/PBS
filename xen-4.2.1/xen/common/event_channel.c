...
static void evtchn_set_pending(struct vcpu *v, int port)
{
                           ...
                           &vcpu_info(v, evtchn_pending_sel)) )
    {
		...
		*/
		/*		
	    rdtscll(now);
		if(port != 0 && port == shared_info(d, target_evtchn))
		{
		//    printk(XENLOG_WARNING "domain %d port %d, current set time: %ld, latency between two set: %ld\n", 
		//				d->domain_id, port, now, now - shared_info(d, pending_set_time));
			shared_info(d, pending_set_time) = now;
		}
		*/
